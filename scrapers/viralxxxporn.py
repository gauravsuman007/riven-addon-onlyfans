"""viralxxxporn.com -- KVS archive of OnlyFans content, indexed by performer.

A sibling of `ultrathots.py`; read that file first for the family's findings --
the two card classes, the `data-original` thumbnails, the substituted gated
rendition. All of them hold here and the helpers below are that file's.

What differs on this deployment, and only this:

**The video path is singular** (``/video/<id>/``), as on hornyfap, while the
model and album paths keep the family's plural forms.

**No scrambling and no token.** The player block carries plain `/get_file/`
URLs; the unscrambler stays for the family's usual reason -- it is a
per-deployment setting that flips on a version bump, and a scrambled URL used
as-is is a well-formed 404 rather than a loud failure.

**The model grid renders preview clips.** Every card carries a
``<id>_preview.mp4`` alongside its thumbnail, and those 404 without the token
that only the video page mints. They are hover previews, not renditions; taking
one as a source publishes a URL that cannot be fetched. `resolve` goes to the
video page, which is where the real renditions are -- verified 206 video/mp4
over byte ranges on both the 720p and 480p entries.

Verified live: the model index pages twenty performers a page, and a model feed
parses with the shared card selector unchanged.
"""
import re
...
class ViralXXXPornScraper(DirectScraper):
    key = "viralxxxporn"
    name = "ViralXXXPorn"
