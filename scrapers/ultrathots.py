"""ultrathots.com -- KVS archive of OnlyFans content, indexed by performer.

This is the reference implementation for the OnlyFans-archive family. Unlike
the tube scrapers, these sites are organised around *people* rather than
releases, and they carry images as well as video -- so this file implements the
optional account methods on `DirectScraper` in addition to `search`/`resolve`.

Four things are worth knowing before editing this file, all found by reading
the live markup rather than by assuming the usual KVS shapes.

**The grids use two different card classes.** The performer index renders
``a.item.second-card`` while the video and album grids render ``a.main-card``.
Selecting on either class alone silently works on one page and returns nothing
on the other. The selectors below key on the *href shape* instead -- an anchor
carrying a ``title`` and pointing at ``/models/<slug>/`` is a performer -- which
is both what actually distinguishes a card and what stays true across the five
sibling sites, whose class names differ but whose URL layouts do not.

**Album pages are themselves paginated.** A 277-photo gallery renders 22
images; the rest arrive through the KVS async block
``custom_album_view_album_view``. Reading only the first page looks like a
working scraper that happens to find small galleries.

**Full-size images are not the ones in the grid.** The grid's
``data-original`` is a 315x421 crop. The real file sits behind
``/get_image/...?i-acctoken=``, the image analogue of ``/get_file/``, which
302s to a signed storage URL. The token is short-lived, which is why
`gallery_images` resolves on open rather than at listing time -- the same split
`DirectSource` exists for.

**Thumbnails are in ``data-original``, not ``src``.** Every card's ``src`` is a
base64 spacer GIF. Using it yields a grid of blank images that parse fine.

The ``function/0/`` unscrambler is not needed by this site today but is kept
for the reason it is kept everywhere else: it is a per-deployment KVS setting
that flips on a version bump, and a scrambled URL used as-is is a well-formed
404 rather than a loud failure. Inlined rather than shared -- plugins must not
import each other (see this repo's AGENTS.md).
"""
...
_COUNT_IN_TITLE_RE = re.compile(r"\((\d+)\s*photos?\)", re.IGNORECASE)


class UltrathotsScraper(DirectScraper):
    key = "ultrathots"
    name = "UltraThots"
    base_url = "https://ultrathots.com"
    ...
    def account_galleries(self, handle: str, page: int = 1) -> list[DirectGallery]:
        response = self._get(
            f"{self.base_url}/albums/models/{handle}/{_page_suffix(page)}",
            params={"sort_by": "post_date"},
        )
        return _galleries(response.text, self.base_url, self.key)

    def gallery_images(self, gallery_id: str) -> list[DirectImage]:
        """Every image this site will serve anonymously for one album.

        There is no paging to do. The album template renders pagination
        controls and the KVS async block accepts a ``from`` parameter, but both
        are decoration here: pages 1, 2 and 3 come back byte-for-byte identical,
        and the full-page and async responses expose the same set. Asking for
        more pages produced the same images with different numbers on them.

        What the site *does* do is gate. A gallery whose own title says "(277
        Photos)" serves six to a signed-out visitor, and the count in
        `DirectGallery.image_count` is the album's real size rather than what
        comes back here. The gap is the site's, not a parsing failure, so this
        returns what is actually reachable rather than retrying to close it.
        """

        response = self._get(f"{self.base_url}/albums/{gallery_id}/-/")
        images = _images(response.text, self.base_url)

        if not images:
            logger.debug(f"{self.key}: no images in album {gallery_id}")
        return images
...
def _galleries(page: str, base_url: str, key: str) -> list[DirectGallery]:
    """Every album card in a grid.

    The image count is read from the card's gallery statistic where the
    template renders one, and otherwise from the title, which these sites write
    as "... (277 Photos)". Neither is guaranteed, so it stays None rather than
    becoming a zero that would read as an empty album.
    """

    tree = lxml_html.fromstring(page)
    galleries: list[DirectGallery] = []
    seen: set[str] = set()
    ...
    for link in tree.xpath("//a[@title and @href]"):
        ...
        title = (link.get("title") or link.text_content()).strip() or "Untitled"
        stated = _COUNT_IN_TITLE_RE.search(title)
        ...
        galleries.append(
            DirectGallery(
                ...
                title=title,
                page_url=urljoin(base_url + "/", href),
                cover=_image_src(link, base_url),
                image_count=(
                    parse_count(_first_text(link, "card-item-text", "images"))
                    or (int(stated.group(1)) if stated else None)
                ),
            )
        )
...
def _images(page: str, base_url: str) -> list[DirectImage]:
    """The full-size image behind every thumbnail on an album page.

    The anchor wrapping each thumbnail is the lightbox target and carries the
    real ``/get_image/`` URL; the ``img`` inside it is only the grid crop.
    """

    tree = lxml_html.fromstring(page)
    images: list[DirectImage] = []

    for link in tree.xpath("//a[@data-fancybox-type='image' and @href]"):
        ...
        images.append(
            DirectImage(
                url=url,
                # The storage host is reached through a signed redirect that
                # does not itself check Referer, but the /get_image/ handler
                # sits on the site and does.
