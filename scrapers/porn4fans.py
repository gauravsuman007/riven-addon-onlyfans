"""porn4fans.com -- KVS archive of OnlyFans content, behind a TLS fingerprint check.

A sibling of `ultrathots.py`; read that file first for the family's findings.

**Plain `requests` gets a 403 here, on every path including the homepage.** As
with `noodlemagazine.py`, the block is on the TLS handshake's fingerprint
rather than on JavaScript, so `curl_cffi` impersonating a real Chrome build
gets a normal 200 on the first request -- no cookie dance and no headless
browser. That is why this file does not use `self.session`.

**The video path is singular** (``/video/<id>/``).

**It has no image galleries.** ``/albums/`` answers 200 but lists nothing, and
``/photos/`` is a 404, so `account_galleries` stays empty.
"""
...
class Porn4FansScraper(DirectScraper):
    key = "porn4fans"
    name = "Porn4Fans"
    base_url = "https://porn4fans.com"
...
def _profile(
    ...
    if not avatar and not bio and not of_username and videos is None and photos is None:
        return None
