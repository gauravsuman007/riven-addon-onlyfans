"""notfans.com -- KVS archive of OnlyFans content, indexed by performer.

A sibling of `ultrathots.py`; read that file first, as it carries the family's
findings (the two card classes, the gated renditions, the lazy-loaded
thumbnails). Only what differs here is written below.

**Media URLs on this deployment are scrambled.** The ``function/0/`` shuffle is
live, not hypothetical, so the unscrambler below is load-bearing rather than
insurance. A scrambled URL used as-is is a well-formed 404, which is why this
would otherwise present as "the scraper stopped working" long after a change.

**This site has real image galleries but cannot attribute them to an account.**
``/albums/`` serves genuine full galleries -- 25 to 28 reachable images, far
more than the six ultrathots exposes -- but no album page links to a model, and
``/albums/models/<handle>/`` answers 200 with an empty grid rather than 404.
There is therefore no way to ask "which galleries belong to this performer",
and `account_galleries` is deliberately left at the base class's empty default.
Implementing it against ``/albums/`` would attach the site's newest galleries
to every account alike, which is worse than showing none: it would look like
data.
"""
...
class NotFansScraper(DirectScraper):
    key = "notfans"
    name = "NotFans"
