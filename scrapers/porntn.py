"""porntn.com -- KVS archive of OnlyFans content, indexed by performer.

A sibling of `ultrathots.py`; read that file first for the family's findings.

**The video path is singular.** ``/video/<id>/`` here against ``/videos/<id>/``
on ultrathots and notfans. The listing regexes below accept either, so only
`resolve` -- which builds a URL rather than reading one -- has to know.

**Its galleries are not worth offering.** ``/albums/`` lists 59 albums and every
one of them exposes exactly one image to a signed-out visitor; there is also no
per-model album path. `account_galleries` stays at the base class's empty
default rather than returning a stream of one-image galleries that would read
as broken parsing.
"""
...
class PornTNScraper(DirectScraper):
    key = "porntn"
    name = "PornTN"
