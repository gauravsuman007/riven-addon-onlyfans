...
#: Every table this add-on owns lives in its own Postgres schema, and that is
#: what makes uninstalling it a single statement:
#:
#:     DROP SCHEMA onlyfans CASCADE
#:
#: Complete by construction -- no enumerating tables, no orphaned sequences,
#: and (because the add-on's ``alembic_version`` is in here too) no migration
#: rows left behind to crash the next startup with a revision nothing can
#: resolve. The host refuses to load an add-on whose metadata says otherwise.
SCHEMA = "onlyfans"

#: A Base of its own, NOT the host's. Sharing the host's would put these
#: tables in its metadata, where `create_all` and autogenerate would both put
#: them back in `public` -- exactly where they cannot be dropped cleanly.
