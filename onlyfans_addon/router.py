...
from onlyfans_addon.registry import reset as reset_of_registry
...
# No prefix of its own: the host mounts this at /api/v1/x/onlyfans and
# applies the API key. An add-on that set its own prefix could collide with
# another add-on's or shadow a host route, so the host owns that decision.
router = APIRouter(tags=["onlyfans"])
...
def sync(sites: Annotated[list[str] | None, Query()] = None) -> SyncStatusResponse:
    ...
    configured = list(addon_settings().sites)
